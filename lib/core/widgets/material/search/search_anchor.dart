...
// TODO: Remove once these changes were merged into stable release.
...
typedef SuggestionsBuilder = Iterable<Widget> Function(
    BuildContext context, SearchController controller);
...
class _SearchViewRoute extends PopupRoute<_SearchViewRoute> {
  ...
  void updateTweens(BuildContext context) {
    ...
    final double endHeight = math.max(effectiveConstraints.minHeight,
        math.min(screenSize.height * 2 / 3, verticalDistanceToEdge));
    ...
    switch (textDirection ?? TextDirection.ltr) {
      case TextDirection.ltr:
        ...
        final double endWidth = math.max(effectiveConstraints.minWidth,
            math.min(anchorRect.width, viewEdgeToScreenEdge));
        final Size endSize = Size(endWidth, endHeight);
        _rectTween.end = showFullScreenView
            ? Offset.zero & screenSize
            : (anchorRect.topLeft & endSize);
        return;
      case TextDirection.rtl:
        ...
        final Offset topLeft =
            Offset(math.max(anchorRect.right - endWidth, 0.0), anchorRect.top);
        final Size endSize = Size(endWidth, endHeight);
        _rectTween.end =
      ...
      child: AnimatedBuilder(
          ...
          builder: (BuildContext context, Widget? child) {
            ...
            final double topPadding = showFullScreenView
                ? lerpDouble(0.0, MediaQuery.of(context).padding.top,
  ...
  void didChangeDependencies() {
    ...
    if (_screenSize != updatedScreenSize) {
      ...
      setState(() {
        ...
        final BoxConstraints constraints = widget.viewConstraints ??
            widget.viewTheme.constraints ??
            widget.viewDefaults.constraints!;
        final Size updatedViewSize = Size(
            math.max(constraints.minWidth, anchorRect.width), _viewRect.height);
        switch (Directionality.of(context)) {
          ...
          case TextDirection.rtl:
            final Offset topLeft = Offset(
                math.max(anchorRect.right - updatedViewSize.width, 0.0),
                anchorRect.top);
...
class SearchViewTheme extends InheritedWidget {
  ...
  static SearchViewThemeData of(BuildContext context) {
    ...
    return searchViewTheme?.data ?? const SearchViewThemeData();
  }
